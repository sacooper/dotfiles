...
# TODO: Use IP address
locations = {}
locations['westford'] = 'USMA0478'
wcr = pywapi.get_weather_from_weather_com(locations['westford'], units='imperial')
conky_loc = '/dotfiles/.conky_hud/weather'
curcond_file = os.environ['HOME'] + conky_loc + '/current_condition'
forecast_file = os.environ['HOME'] + conky_loc + '/forecast'
icons_path = os.environ['HOME'] + conky_loc + '/weather_icons/'
_icons_path = os.environ['HOME'] + conky_loc + '/weather_icons/'
...
wtr = {
        ...
        'BAR' : cur_cond['barometer']['reading'] + units['pressure'],
        'DEW' : cur_cond['dewpoint'] + '°' + units['temperature'],
        'WND' : (wind['speed'] + ' ' + units['speed'] + ' from ' + wind['text']) if wind['speed'] != 'calm' else wind['text'],
...
for elements in forcast:
    ...
    cond.append(os.environ['HOME'] + conky_loc + '/weather_icons/day/' + elements['day']['text'].lower().replace(' ', '') + '.png')
    cond[0] = os.environ['HOME'] + conky_loc + '/weather_icons/day/' + wtr['TXT'] + '.png'
...
def main():
    ...
    print('${goto 60}' + '${voffset 25}' + fontIt('Hack', '13', wtr['FLK'] +\
            '\n' +\
            '\n' + '${goto 60}' + wtr['HUM'] +\
            '\n' +\
            '\n' + '${goto 60}' + wtr['BAR'] +\
            ...
            imageIt(forecasts_ready['cond'][0], 20, 413, 15, 15) +\
            imageIt(forecasts_ready['cond'][1], 51, 413, 15, 15) +\
            imageIt(forecasts_ready['cond'][2], 82, 413, 15, 15) +\
            imageIt(forecasts_ready['cond'][3], 113, 413, 15, 15) +\
            imageIt(forecasts_ready['cond'][4], 144, 413, 15, 15) +\
            '\n')
    print('${color3}└┈─                ─┈┘${color}')
